...
const MasterFooter = ({ course }: MasterHeaderProps) => {
  ...
  const onLoad = (unlayer: any) => {
    ...
    unlayer.loadDesign(course.footerJson);
  };
    ...
    unlayer?.exportHtml(async (data: any) => {
      ...
      try {
        const response = await axios.put(
          `/api/course/${course.id}/master/footer`,
          { footerJsonContent: design, footerHtml: html }
        );
    ...
    <div className="w-full flex justify-between flex-col ">
      <div className=" flex justify-between mb-2  ">
        <div className="text-sm ">
          Set footer for all chapters
          <pre className=" text-xs  text-rose-500">
            Check all chapters to see changes.
          </pre>
        </div>
